...
/* registry hive ids */
, HKLM = 'HKLM', HKCU = 'HKCU', HKCR = 'HKCR', HKU = 'HKU', HKCC = 'HKCC', HIVES = [HKLM, HKCU, HKCR, HKU, HKCC]
/* registry value type ids */
, REG_SZ = 'REG_SZ', REG_MULTI_SZ = 'REG_MULTI_SZ', REG_EXPAND_SZ = 'REG_EXPAND_SZ', REG_DWORD = 'REG_DWORD', REG_QWORD = 'REG_QWORD', REG_BINARY = 'REG_BINARY', REG_NONE = 'REG_NONE', REG_TYPES = [REG_SZ, REG_MULTI_SZ, REG_EXPAND_SZ, REG_DWORD, REG_QWORD, REG_BINARY, REG_NONE]
...
export function Registry(options) {
    ...
    /* private members */
    let _options = options || {}, _host = '' + (_options.host || '') // hostname
...
Registry.prototype.values = function values(cb) {
    ...
    }), buffer = '', self = this, error = null; // null means no error previously reported.
    ...
    proc.on('close', function (code) {
        if (error) {
            ...
            let items = [], result = [], lines = buffer.split('\n'), lineNumber = 0;
            ...
            for (let i = 0, l = items.length; i < l; i++) {
                let match = ITEM_PATTERN.exec(items[i]), name, type, value;
                if (match) {
                    name = match[1].trim();
                    type = match[2].trim();
                    value = match[3];
...
Registry.prototype.keys = function keys(cb) {
    ...
    proc.stdout.on('end', function () {
        ...
        for (let i = 0, l = items.length; i < l; i++) {
            let match = PATH_PATTERN.exec(items[i]), hive, key;
            if (match) {
                hive = match[1];
                key = match[2];
                if (key && (key !== self.key)) {
...
Registry.prototype.get = function get(name, cb) {
    ...
    proc.on('close', function (code) {
        if (error) {
            ...
            let items = [], result = null, lines = buffer.split('\n'), lineNumber = 0;
            for (let i = 0, l = lines.length; i < l; i++) {
                let line = lines[i].trim();
                if (line.length > 0) {
                    log(line);
            ...
            let item = items[items.length - 1] || '', match = ITEM_PATTERN.exec(item), name, type, value;
            if (match) {
                name = match[1].trim();
                type = match[2].trim();
                value = match[3];
